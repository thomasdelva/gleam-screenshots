...
| `.github/workflows/ci.yml` | This repo's own self-contained CI: one `gleam` job (format check + the screenshot suite on both targets, sharing one setup-beam) that also folds in the label-gated accept as a plain `git` commit step. |
